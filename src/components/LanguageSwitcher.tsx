...
const LanguageSwitcher = ({ changeLanguage }: LanguageSwitcherProps) => {
    return (
        <div className='fixed bottom-0 right-0 m-2'>
            <img src={enFlag} alt="EN" onClick={() => changeLanguage('en')} className={(getCurrentLanguage() === Language.English ? "hidden" : "inline") + " w-8"} />
            <img src={frFlag} alt="FR" onClick={() => changeLanguage('fr')} className={(getCurrentLanguage() === Language.French ? "hidden" : "inline") + " w-8"} />
        </div>
    )
}
